"""Consolidacion Inicial de Esquema Completo - EduAgro V1

Revision ID: 001_initial_full_schema
Revises: 
...
revision: str = "001_initial_full_schema"
...
def upgrade() -> None:
    # 1. ENUMs
    enum_statements = [
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'rol_usuario_enum') THEN CREATE TYPE rol_usuario_enum AS ENUM ('administrador', 'productor', 'contador', 'operador', 'invitado'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_servicio_enum') THEN CREATE TYPE tipo_servicio_enum AS ENUM ('luz_rural', 'internet', 'combustible', 'mantenimiento', 'impuesto_tasa'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'frecuencia_pago_enum') THEN CREATE TYPE frecuencia_pago_enum AS ENUM ('mensual', 'bimensual', 'trimestral', 'semestral', 'anual', '2_anios', '3_anios', 'eventual'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_servicio_instalado_enum') THEN CREATE TYPE estado_servicio_instalado_enum AS ENUM ('al_dia', 'vencido', 'pendiente', 'suspendido'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_servicio_enum') THEN CREATE TYPE estado_servicio_enum AS ENUM ('pendiente', 'pagado', 'vencido', 'anulado'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'document_type_enum') THEN CREATE TYPE document_type_enum AS ENUM ('factura', 'recibo', 'presupuesto', 'contrato', 'comprobante_pago', 'otro'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tenencia_tipo_enum') THEN CREATE TYPE tenencia_tipo_enum AS ENUM ('propio', 'alquilado', 'aparceria'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_labor_enum') THEN CREATE TYPE tipo_labor_enum AS ENUM ('siembra', 'fertilizacion', 'fumigacion', 'cosecha', 'monitoreo', 'otro'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_cpe_enum') THEN CREATE TYPE estado_cpe_enum AS ENUM ('borrador', 'en_transito', 'descargado', 'anulado'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_transaccion_enum') THEN CREATE TYPE tipo_transaccion_enum AS ENUM ('ingreso', 'egreso'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_precio_enum') THEN CREATE TYPE tipo_precio_enum AS ENUM ('hecho', 'a_fijar', 'canje'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ubicacion_stock_enum') THEN CREATE TYPE ubicacion_stock_enum AS ENUM ('silo_propio', 'silobolsa', 'acopio_terceros'); END IF; END $$;",
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_compromiso_enum') THEN CREATE TYPE tipo_compromiso_enum AS ENUM ('contrato_venta', 'arrendamiento_fijo', 'canje_insumos', 'reserva_semilla', 'otro'); END IF; END $$;",
    ]
    ...
    tables = [
        ...
        """CREATE TABLE IF NOT EXISTS stock_quality_measurements (
            id UUID PRIMARY KEY,
            cliente_id UUID NOT NULL REFERENCES clientes(id) ON DELETE CASCADE,
            stock_partida_id UUID NOT NULL REFERENCES stock_partidas(id) ON DELETE CASCADE,
            ...
            estado_calidad VARCHAR(50) NOT NULL DEFAULT 'apto',
            fuente VARCHAR(50) NOT NULL DEFAULT 'propia',
            observaciones TEXT,
            created_by_user_id UUID,
            fecha_creacion TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
        )""",
    ...
    # 3. COLUMNAS ADICIONALES IDEMPOTENTES
    columns = [
