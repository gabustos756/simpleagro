...
class Lote(Base):
    ...
    geometria_geojson: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
    superficie_calculada_gis_ha: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    perimetro_calculado_m: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    centroide_lat: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    centroide_lng: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    fuente_geometria: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, default="DIBUJO_MANUAL")
    fecha_actualizacion_geometria: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
